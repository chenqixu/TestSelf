...
public class HttpUtil {
    ...
    public HttpEntity doGetEntity(String url, String ktName) {
        Map<String, String> headerMap = new HashMap<>();
        final HttpEntity[] result = new HttpEntity[1];
        final String _ktName = ktName;
        doSend(url, null, null, HttpGet.class, headerMap, new IHttpEntityDeal() {
            @Override
            public void deal(HttpEntity entity) {
                try {
                    // 通过EntityUtils中的toString方法将结果转换为字符串
                    FileOutputStream fos = new FileOutputStream(ParameterConfig.getValue("KEYTAB_FILE_DIR") + _ktName + ".keytab");
                    entity.writeTo(fos);
                    result[0] = entity;
                } catch (IOException e) {
