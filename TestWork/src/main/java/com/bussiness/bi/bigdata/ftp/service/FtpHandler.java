...
public class FtpHandler extends Thread {
	...
	public void run() {
		...
		try {
			BufferedReader ctrlInput = new BufferedReader(
					new InputStreamReader(ctrlSocket.getInputStream(), Charset.forName(LANG)));
	...
	boolean commandUSER() {
		if (cmd.equals("USER")) {
			reply = "331 User name is correct, need password";
			user = param;
	...
	boolean commandPASS() {
		if (cmd.equals("PASS")) {
			if (checkPASS(param)) {
				...
				System.out.println("New message: user: " + user + " from: " + remoteHost
						+ "logged in");
				System.out.print("->");
				return false;
			} else {
				reply = "530 not logged on";
				return true;
			}
		} else {
		...
		if (f.isDirectory() && f.exists()) {
			if (param.equals("..") || param.equals("..\\")) {
				if (dir.compareToIgnoreCase(rootdir) == 0) {
					reply = "550 This path does not exist";
					// return false;
				} else {
					s1 = new File(dir).getParent();
					if (s1 != null) {
						dir = s1;
						reply = "250 Requested file processing ends, the current directory becomes: " + dir;
					} else
						reply = "550 This path does not exist";
				}
				...
				dir = param;
				reply = "250 The file processing is finished, the work path becomes " + dir;
			}
		} else if (f1.isDirectory() && f1.exists()) {
			dir = s + param;
			reply = "250 The file processing is finished, the work path becomes " + dir;
		} else
			reply = "501 Parameter syntax error";

		return false;
	} // commandCDW() end

	boolean commandQUIT() {
		reply = "221 Service close connection";
		return true;
	}// commandQuit() end
	...
	boolean commandLIST()// 文件和目录的列表
	{
		try {
			...
			PrintWriter dout = new PrintWriter(dataSocket.getOutputStream(),
					true);
			if (param.equals("") || param.equals("LIST")) {
				ctrlOutput.println("150 File status is normal, ASCII to LS mode operation");
				File f = new File(dir);
			...
			dout.close();
			dataSocket.close();
			reply = "226 End of transmission data connection";
		} catch (Exception e) {
			e.printStackTrace();
	...
	boolean commandRETR() {
		...
		if (isrest) {
			...
			if (type == FtpState.FTYPE_IMAGE) // bin
			{
				try {
					ctrlOutput
							.println("150 File status is normal, open the file in two ways:  " + requestfile);
					dataSocket = new Socket(remoteHost, remotePort,
							InetAddress.getLocalHost(), 20);
	...
	boolean commandPWD() {
		reply = "257 " + dir + " Is the current directory.";
		return false;
	}
