...
public class TimeTest {
	 ...
	 * 2个时间字符串进行比较，返回
	 * */
	public static boolean dateCompareTo(String date1, String date2, String format){
    ...
    public static String formatDateString(String date, String origFormat, String destFormat) throws ParseException{
	 ...
	 * */
	public static long getSplitTime(String date1_s, String date1_e, String date2_s, String date2_e, String format){
		SimpleDateFormat sdf = new SimpleDateFormat(format);
		long result = 0;
		if(date1_s!=null && date1_e!=null && date2_s!=null && date2_e!=null
				&& date1_s.trim().length()>0 && date1_e.trim().length()>0
				&& date2_s.trim().length()>0 && date2_e.trim().length()>0){
			try {
				//开始时间大于查询开始时间，结束时间小于等于查询结束时间
				if(sdf.parse(date1_s).after(sdf.parse(date2_s))&&sdf.parse(date1_e).before(sdf.parse(date2_e))){
					result = sdf.parse(date1_e).getTime()-sdf.parse(date1_s).getTime();
				}
				//开始时间小于查询开始时间，结束时间小于等于查询结束时间
				else if(sdf.parse(date1_s).before(sdf.parse(date2_s))&&
						(sdf.parse(date1_e).before(sdf.parse(date2_e)) || sdf.parse(date1_e).equals(sdf.parse(date2_e)))){
					result = sdf.parse(date1_e).getTime()-sdf.parse(date2_s).getTime();
				}
				//开始时间大于等于查询开始时间，结束时间大于查询结束时间
				else if((sdf.parse(date1_s).after(sdf.parse(date2_s)) || sdf.parse(date1_s).equals(sdf.parse(date2_s)))&&
						sdf.parse(date1_e).after(sdf.parse(date2_e))){
					result = sdf.parse(date2_e).getTime()-sdf.parse(date1_s).getTime();
				}
				//开始时间小于等于查询开始时间，结束时间大于等于查询结束时间
				else if((sdf.parse(date1_s).before(sdf.parse(date2_s)) || sdf.parse(date1_s).equals(sdf.parse(date2_s)))&&
						(sdf.parse(date1_e).after(sdf.parse(date2_e)) || sdf.parse(date1_e).equals(sdf.parse(date2_e)))){
					result = sdf.parse(date2_e).getTime()-sdf.parse(date2_s).getTime();
				}
			} catch (ParseException e) {
				e.printStackTrace();
			}
...
//		long c = getSplitTime("20160216010639","20160216010649","20160216010644","20160216010649","yyyyMMddHHmmss");
