 ...
 * @date:2014-02-19
 */
public class DataCollectProcess {
	//日志记录器
		...
		try {
			...
			while(true){
				...
				if(InitCollectorFile.ifNeedExitFlag 
					...
					if(f.exists())
						logger.info("#####采集程序开始进行第"+InitCollectorFile.currCycleNum+"次循环扫描目录时，检测到退出文件："+CollectorConstant.EXIT_FILE_FULL_NAME+" 存在，采集程序退出>>>>>>>>>>>>>>>>");
					if(InitCollectorFile.ifNeedExitFlag)
						logger.info("#####采集程序开始进行第"+InitCollectorFile.currCycleNum+"次循环扫描目录时，全局变量退出标志为真，采集程序退出>>>>>>>>>>>>>>>>");
					return;
				}
				//如果配置文件配置为SFTP采集
