    		...
    		sb.group(group) // 绑定线程池
    			...
    			.childHandler(new ChannelInitializer<SocketChannel>() { // 绑定客户端连接时候触发操作
    				...
    				protected void initChannel(SocketChannel ch) throws Exception {
    					...
    					ch.pipeline().addLast(new DiscardServerHandler(taskPool)); // 客户端触发操作
    				}
    			});
    		ChannelFuture cf = sb.bind().sync(); // 服务器异步创建绑定
    		System.out.println(DiscardServer.class + " started and listen on " + cf.channel().localAddress());
        ...
        try {
            ...
            b = b.childHandler(new ChannelInitializer<SocketChannel>() { // (4)
                ...
                public void initChannel(SocketChannel ch) throws Exception {
                    ch.pipeline().addLast(new DiscardServerHandler(taskPool));// demo1.discard
